 ...
 */
function resolveBodyFileOption(options) {
    if (options.bodyFile && typeof options.bodyFile === "string") {
        const source = options.bodyFile;
        // stdin 以外はファイル存在チェック
...
program
    ...
    if (!resolveBodyFileOption(options))
        return;
    // エイリアスオプションのマージ (#587)
...
program
    ...
    .description("GitHub Issues 管理 with Projects (list, show, create, update, comment, comment-edit, close, reopen, import, fields, remove, search, pr-create, pr-comments, merge, pr-reply, resolve, sub-list, sub-add, sub-remove)")
    ...
    .option("--state-reason <reason>", "Close reason: COMPLETED, NOT_PLANNED (for close)", "COMPLETED")
    .option("--add-assignee <user>", "Add assignee to issue, @me for self (for update)")
