 ...
 */
async function findCategory(owner, repo, categoryName) {
 ...
 */
async function getDiscussionId(owner, repo, number) {
...
async function cmdGet(idOrNumber, options, logger) {
    ...
    if (/^\d+$/.test(idOrNumber)) {
        ...
        const result = await runGraphQL(GRAPHQL_QUERY_DISCUSSION_BY_ID, {
            id: idOrNumber,
        });
...
async function cmdUpdate(idOrNumber, options, logger) {
    ...
    const result = await runGraphQL(GRAPHQL_MUTATION_UPDATE_DISCUSSION, {
        discussionId,
 ...
 */
export async function discussionsCommand(action, target, options) {
