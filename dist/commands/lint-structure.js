...
import { checkDirRequired, checkFileRequired, checkLibNoRootFiles, checkLibHasIndex, checkDirRecommended, checkNamingConvention, checkNoCrossAppImport, checkActionsStructure, checkComponentsDomainGrouping, checkLibStructureCompliance, checkBarrelExportRequired, checkActionsSeparation, } from "../lint/rules/structure-rules.js";
/**
 * lint-structure コマンドハンドラ
 */
export async function lintStructureCommand(options) {
    const logger = createLogger(options.verbose);
    const projectPath = resolve(options.project);
    logger.info("プロジェクト構造を検証中");
    // 設定読み込み
    const config = loadConfig(projectPath, options.config);
    const lintStructureConfig = config.lintStructure;
    ...
    if (report.summary.passed) {
        ...
        if (strict) {
            ...
            logger.warn(`構造検証完了 - ${report.summary.errors}件のエラー（non-strictモード）`);
            process.exit(0);
        }
    }
}
 ...
 * lint-structure を実行
 */
function runLintStructure(projectPath, config, projectName) {
    ...
    if (existsSync(packagesPath)) {
        const pkgDirs = readdirSync(packagesPath).filter((name) => statSync(join(packagesPath, name)).isDirectory());
 ...
 * アプリの構造チェックを実行
 */
function runAppChecks(projectPath, appName, appPath, rules) {
    const checks = [];
    // 1. 必須ディレクトリ
    ...
    if (rules?.["no-cross-app-import"]) {
        checks.push(...checkNoCrossAppImport(projectPath, appName, rules["no-cross-app-import"]));
    }
    // 8. actions/ 構造チェック
    checks.push(...checkActionsStructure(appPath, "info"));
    // 9. components/ ドメイン別グループ化
    if (rules?.["components-domain-grouping"]) {
        checks.push(...checkComponentsDomainGrouping(appPath, rules["components-domain-grouping"]));
    }
    // 10. lib/ 構造準拠チェック
    if (rules?.["lib-structure-compliance"]) {
        checks.push(...checkLibStructureCompliance(appPath, rules["lib-structure-compliance"]));
    }
    // 11. バレルエクスポート必須チェック
    if (rules?.["barrel-export-required"]) {
        checks.push(...checkBarrelExportRequired(appPath, rules["barrel-export-required"]));
    }
    // 12. actions分離チェック
    if (rules?.["actions-separation"]) {
        checks.push(...checkActionsSeparation(appPath, rules["actions-separation"]));
    }
    return checks;
}
/**
 * パッケージの構造チェックを実行
 */
function runPackageChecks(pkgPath, pkgName, rules) {
    const checks = [];
    // パッケージ固有の必須ディレクトリ
    ...
    if (rules?.["file-required"]?.packages?.[pkgName]) {
        ...
        checks.push(...checkFileRequired(pkgPath, files, rules["file-required"].severity));
    }
    return checks;
}
/**
 * 推奨アクションを生成
 */
function generateRecommendedActions(checks) {
    const actions = [];
    let priority = 1;
    // エラーを優先
    ...
    if (report.apps.length > 0) {
        ...
        for (const app of report.apps) {
            ...
            for (const issue of issues) {
                const issueIcon = issue.status === "error"
                    ? "\u274C"
...
//# sourceMappingURL=lint-structure.js.map