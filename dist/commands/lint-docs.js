 ...
 * lint-docs コマンドハンドラ
 */
export async function lintDocsCommand(options) {
 ...
 * lint-docs を実行
 */
async function runLintDocs(projectPath, config, logger) {
    const fileResults = [];
    const patternResults = [];
    for (const reqConfig of config.required) {
        if ("file" in reqConfig) {
            // 単一ファイル検証
            const result = await validateSingleFile(projectPath, reqConfig, config, logger);
            fileResults.push(result);
        }
        else if ("filePattern" in reqConfig) {
            // パターン検証
            const result = await validateFilePattern(projectPath, reqConfig, config, logger);
 ...
 * 単一ファイルを検証
 */
async function validateSingleFile(projectPath, config, lintConfig, logger) {
 ...
 * ファイルパターンを検証
 */
async function validateFilePattern(projectPath, config, lintConfig, logger) {
...
//# sourceMappingURL=lint-docs.js.map