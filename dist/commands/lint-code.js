 ...
 * デフォルト設定
 */
const defaultServerActionsConfig = {
...
export async function lintCodeCommand(options) {
    ...
    if (report.passed) {
        ...
        if (strict) {
            ...
            logger.warn(`コード検証完了 - ${report.summary.errorCount}件のエラー（non-strictモード）`);
            process.exit(0);
        }
    }
}
 ...
 * lint-code を実行
 */
async function runLintCode(projectPath, config, logger) {
    const fileResults = [];
    // Server Actions 検証
 ...
 * Server Actions を検証
 */
async function validateServerActions(projectPath, config, logger) {
    const results = [];
    // ファイル収集
 ...
 */
function validateServerActionFile(content, filePath, config) {
    const issues = [];
    // モジュールヘッダー検証
    const moduleHeader = validateModuleHeader(content, filePath, config.requiredFileHeader);
    issues.push(...moduleHeader.issues);
    // 関数検証
    const functions = validateFunctions(content, filePath, config.requiredFunctionTags);
    ...
    for (const fn of functions) {
        const structureIssues = serverActionStructureRule.check(content, filePath, fn.name, fn.line);
 ...
 * モジュールヘッダーを検証
 */
function validateModuleHeader(content, filePath, requiredTags) {
    const issues = [];
    ...
    const headerAreaMatch = content.match(/^["']use server["']\s*(\/\*\*[\s\S]*?\*\/)/);
    if (!headerAreaMatch) {
        issues.push({
            type: "error",
 ...
 * 関数を検証
 */
function validateFunctions(content, filePath, requiredTags) {
    const results = [];
 ...
 * セクション区切りを検証
 */
function validateSectionSeparators(content, filePath) {
    const issues = [];
    // セクション区切りパターン
...
function formatTerminal(report) {
    ...
    for (const fr of report.fileResults) {
        ...
        if (!fr.moduleHeader.exists) {
            ...
            lines.push(`   \u26A0\uFE0F  Missing header tags: ${fr.moduleHeader.missingTags.join(", ")}`);
        }
        // 関数情報
...
//# sourceMappingURL=lint-code.js.map