...
import { GRAPHQL_MUTATION_CREATE_DISCUSSION, GRAPHQL_MUTATION_CLOSE_ISSUE, getRepoId, } from "../utils/graphql-queries.js";
...
// =============================================================================
async function getHandoversCategoryId(owner, repo, categoryName) {
 ...
 */
async function fetchLatestHandover(owner, repo, categoryId, authorFilter) {
 ...
 */
async function fetchTeamHandovers(owner, repo, categoryId) {
    const result = await runGraphQL(GRAPHQL_QUERY_RECENT_HANDOVERS, {
        owner,
        name: repo,
        categoryId,
...
async function fetchActiveIssues(owner, repo, limit, states = ["OPEN"]) {
    const allIssues = [];
    let cursor = null;
...
// =============================================================================
async function updateIssueStatus(projectId, itemId, statusValue, projectFields, logger) {
...
export function generatePreflightWarnings(git, sessionBackups) {
    ...
    if (git.branch && PROTECTED_BRANCHES.includes(git.branch)) {
        warnings.push(`On protected branch "${git.branch}". Create a feature branch before committing.`);
    }
    if (git.hasUncommittedChanges) {
        warnings.push(`${git.uncommittedChanges.length} uncommitted change(s) detected.`);
    }
    if (git.unpushedCommits !== null && git.unpushedCommits > 0) {
        warnings.push(`${git.unpushedCommits} unpushed commit(s). Push before ending session.`);
    }
    if (sessionBackups > 0) {
 ...
 */
async function cmdPreflight(options, logger) {
    const config = loadGhConfig();
    const repoInfo = getRepoInfo();
    if (!repoInfo) {
        logger.error("Could not determine repository");
 ...
 */
async function fetchItemTextFieldValues(projectId) {
...
// =============================================================================
async function closeIssueById(issueId) {
    ...
    if (options.fix && inconsistencies.length > 0) {
        ...
        for (const item of errorItems) {
            if (item.issueState === "OPEN") {
                ...
                const success = await updateIssueStatus(issueData.projectId, issueData.projectItemId, "Done", fields, logger);
                fixes.push({
                    number: item.number,
                    action: "update-status",
                    success,
