 ...
 */
import { runGhCommand, runGhCommandRaw, runGraphQL, isIssueNumber, parseIssueNumber, validateBody, } from "../utils/github.js";
import { formatOutput } from "../utils/formatters.js";
import { resolveTargetRepo, } from "../utils/repo-pairs.js";
import { getProjectId, setItemFields, cmdGetIssueDetail, } from "./gh-issues.js";
// =============================================================================
...
export async function cmdPrComments(prNumberStr, options, logger) {
    ...
    const result = runGraphQL(GRAPHQL_QUERY_PR_REVIEW_THREADS, {
        owner,
        name: repo,
        number: prNumber,
    });
...
export async function cmdMerge(prNumberStr, options, logger) {
    ...
    const prBodyResult = runGhCommand(["pr", "view", String(prNumber), "--json", "body", "--repo", `${owner}/${repo}`], { silent: true });
        ...
        if (projectId) {
            for (const num of linkedNumbers) {
                ...
                if (!detail?.projectItemId)
                    continue;
                const fieldResult = setItemFields(projectId, detail.projectItemId, { Status: "Done" }, logger);
                if (fieldResult > 0) {
...
export async function cmdResolve(prNumberStr, options, logger) {
    ...
    const prNumber = parseIssueNumber(prNumberStr);
    const result = runGraphQL(GRAPHQL_MUTATION_RESOLVE_THREAD, {
 ...
 * 見つからない場合は null を返す。
 */
export function resolvePrFromHead(headBranch, owner, repo, logger) {
    const result = runGhCommand(["pr", "list", "--head", headBranch, "--json", "number,url", "--repo", `${owner}/${repo}`], { silent: true });
...
// =============================================================================
export function fetchOpenPRs(owner, repo, limit = 10) {
    const result = runGraphQL(GRAPHQL_QUERY_OPEN_PRS, {
        owner,
        name: repo,
        first: limit,
    });
...
//# sourceMappingURL=gh-issues-pr.js.map