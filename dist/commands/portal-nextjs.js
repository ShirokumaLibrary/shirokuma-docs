...
import { existsSync, rmSync, mkdirSync, copyFileSync, readdirSync, writeFileSync, readFileSync } from "node:fs";
 ...
 * Next.js ポータルをビルド
 */
export async function buildNextjsPortal(options) {
    ...
    try {
        ...
        if (existsSync(applicationsPath)) {
            try {
                ...
                hasApiApps = appData?.apps?.some((app) => app.type === "api") ?? false;
            }
            catch {
                hasApiApps = false;
            }
 ...
 * 生成されたファイル一覧を取得
 */
function getGeneratedFiles(dir, prefix = "") {
    const files = [];
    if (!existsSync(dir))
        return files;
...
//# sourceMappingURL=portal-nextjs.js.map