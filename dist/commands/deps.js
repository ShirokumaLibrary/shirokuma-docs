 ...
 */
async function checkDepcruise(projectPath, logger) {
 ...
 */
async function checkGraphviz(logger) {
 ...
 */
function buildIncludeRegex(patterns) {
    return patterns.map((p) => `^${p}`).join("|");
}
/**
 * JSON 形式で依存関係を出力
 */
async function generateJson(projectPath, existingPaths, excludeRegex, outputPath, logger, verbose) {
    try {
        const args = [
            "depcruise",
            ...existingPaths,
 ...
 * SVG 形式で依存関係グラフを出力
 */
async function generateSvg(projectPath, existingPaths, excludeRegex, outputPath, logger, verbose) {
    // graphviz が必要
...
export async function depsCommand(options) {
    ...
    for (const format of formats) {
        switch (format) {
            case "json": {
                ...
                const success = await generateJson(projectPath, existingPaths, excludeRegex, jsonPath, logger, options.verbose || false);
                results.push({ format: "json", success });
                break;
            }
            case "svg": {
                const svgPath = resolve(outputDir, "dependencies.svg");
                const success = await generateSvg(projectPath, existingPaths, excludeRegex, svgPath, logger, options.verbose || false);
    ...
    if (successCount > 0) {
        logger.success(`依存関係グラフ生成完了 (成功: ${successCount}, 失敗: ${failCount})`);
    }
    else {
