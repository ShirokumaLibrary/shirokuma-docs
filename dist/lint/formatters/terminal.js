...
export function formatTerminal(report) {
    ...
    for (const result of report.results) {
        if (result.issues.length === 0) {
            lines.push(chalk.green("pass ") + chalk.gray(result.file) + chalk.gray(` (${result.totalTests} tests)`));
        }
        else {
            lines.push(chalk.yellow("warn ") + chalk.white(result.file) + chalk.gray(` (${result.totalTests} tests)`));
            for (const issue of result.issues) {
                const color = getSeverityColor(issue.severity);
                const symbol = getSeveritySymbol(issue.severity);
                lines.push(`  ${color(symbol)} ${chalk.gray(`L${issue.line}:`)} ${issue.message} ${chalk.gray(`(${issue.rule})`)}`);
            }
            lines.push("");
        }
    }
