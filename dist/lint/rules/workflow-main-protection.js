 ...
 */
export function validateMainProtection(state, severity = "error", protectedBranches = DEFAULT_PROTECTED_BRANCHES) {
    const issues = [];
    ...
    for (const commit of state.recentCommits) {
        if (/Co-Authored-By:/i.test(commit.body) ||
 ...
 */
export async function checkMainProtection(severity = "error", protectedBranches = DEFAULT_PROTECTED_BRANCHES) {
    ...
    return validateMainProtection({ currentBranch, hasUncommittedChanges, directCommitCount, recentCommits }, severity, protectedBranches);
}
//# sourceMappingURL=workflow-main-protection.js.map