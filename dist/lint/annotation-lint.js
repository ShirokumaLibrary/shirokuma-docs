 ...
 * ```
 */
export function extractUsedComponentsAnnotation(content) {
    // JSDocブロックを探す
    ...
    for (const jsdoc of jsdocMatches) {
        ...
        const usedComponentsMatch = jsdoc.match(/@usedComponents\s+([^@]*?)(?=\s*(?:@|\*\/))/s);
 ...
 * ```
 */
export function extractComponentsFromImports(content, options = {}) {
 ...
 * ```
 */
export function compareUsedComponents(annotated, imported) {
 ...
 * ```
 */
export function checkScreenAnnotation(content, filePath, options = {}) {
 ...
 * ```
 */
export function checkComponentAnnotation(content, filePath, options = {}) {
    const { exclude = [] } = options;
    const errors = [];
    const warnings = [];
    const infos = [];
 ...
 * ```
 */
export function generateScreenName(filePath) {
    // ファイルパスからルート部分を抽出
 ...
 * ```
 */
export function generateRoute(filePath) {
    const parts = filePath.split("/");
    // app ディレクトリ以降を取得
    const appIndex = parts.findIndex((p) => p === "app");
    if (appIndex === -1) {
 ...
 * JSDocコメントブロックを検出
 */
function findFirstJSDoc(content) {
 ...
 * JSDocにタグを追加または更新
 */
function addOrUpdateJSDocTag(jsdocText, tag, value) {
    // 既存のタグを検索
 ...
 * @returns 修正結果
 */
export function fixUsedComponentsAnnotation(content, filePath) {
    // インポートからコンポーネントを抽出
    ...
    // 既存のアノテーションと同じなら変更なし
    if (existingAnnotation.length === components.length &&
    ...
    if (jsdoc) {
        // 既存のJSDocにタグを追加/更新
        const updatedJSDoc = addOrUpdateJSDocTag(jsdoc.text, "usedComponents", componentsList);
        const newContent = content.slice(0, jsdoc.start) +
            updatedJSDoc +
 ...
 * @returns 修正結果
 */
export function fixRouteAnnotation(content, filePath) {
    // 既存の @route があれば変更なし
 ...
 * @returns 修正結果
 */
export function applyFixes(content, filePath, options) {
...
//# sourceMappingURL=annotation-lint.js.map