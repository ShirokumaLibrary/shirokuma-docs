...
export interface ApiToolDef {
  ...
  relatedTests?: string;
  tests?: ApiToolTest[];
  // Test coverage (like details.json)
  testCoverage?: {
    hasTest: boolean;
    totalTests: number;
    coverageScore: number;
  };
  // Swagger-style display
 ...
 */
function inferHttpMethod(toolName: string): HttpMethod {
 ...
 */
function extractJsDocTags(comment: string): Record<string, string> {
 ...
 */
function parseFileHeaderAnnotations(sourceCode: string): ApiToolAnnotation {
  ...
  return {
    ...
    authLevel: tags.authLevel as ApiToolAnnotation["authLevel"],
 ...
 */
function parseApiToolsFromSource(filePath: string, sourceCode: string): ApiToolDef[] {
 ...
 */
function generateApiToolsHtml(data: ApiToolsOutput): string {
  const toolsByCategory: Record<string, ApiToolDef[]> = {};
...
export async function runApiTools(options: ApiToolsOptions): Promise<void> {
  ...
  let _config: ShirokumaConfig | null = null;
  try {
  ...
  for (const tool of allTools) {
    ...
    if (testCount > 0) {
      ...
      if (testCount >= 5) score += 20;
      // Bonus for DB integration tests
      if (hasDb && testCount >= 2) score += 10;
      // Bonus for auth testing
      if (hasAuth && testCount >= 2) score += 10;
