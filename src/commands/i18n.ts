 ...
 * デフォルト設定を取得
 */
function getDefaultI18nConfig(): ResolvedI18nConfig {
  return {
    enabled: true,
 ...
 * i18n コマンドハンドラ
 */
export async function i18nCommand(options: I18nOptions): Promise<void> {
  ...
  const configWithI18n = config as unknown as { i18n?: Partial<ResolvedI18nConfig> };
 ...
 * メッセージファイルを収集
 */
function collectMessageFiles(projectPath: string, config: ResolvedI18nConfig): string[] {
 ...
 */
function parseFilePath(filePath: string): { app: string | null; locale: string; namespace: string } | null {
...
function parseMessageFiles(
  ...
  locales: string[];
  apps: string[];
  namespaceData: Map<string, { app: string | null; namespace: string; localeData: Map<string, Record<string, unknown>> }>;
} {
  const localesSet = new Set<string>();
  const appsSet = new Set<string>();
  // key -> { app, namespace, localeData: locale -> data }
  const namespaceData = new Map<string, { app: string | null; namespace: string; localeData: Map<string, Record<string, unknown>> }>();
 ...
 * アプリ名からメタ情報を生成
 */
function getAppMeta(appId: string): { name: string; icon: string; color: string } {
  const appMeta: Record<string, { name: string; icon: string; color: string }> = {
 ...
 * i18n ドキュメントを構築
 */
function buildI18nDocumentation(
  locales: string[],
  apps: string[],
  namespaceData: Map<string, { app: string | null; namespace: string; localeData: Map<string, Record<string, unknown>> }>,
  primaryLocale: string
  ...
  // アプリ情報を集計
  const appStats = new Map<string, { namespaceCount: number; keyCount: number }>();
  ...
  // アプリリストを生成
  const i18nApps: I18nApp[] = apps.map((appId) => {
 ...
 * i18n 一覧ページ HTML を生成
 */
function generateI18nListHtml(doc: I18nDocumentation, projectName: string): string {
 ...
 */
function generateNamespaceDetailHtml(
