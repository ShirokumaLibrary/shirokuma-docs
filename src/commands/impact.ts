...
export async function impactCommand(options: ImpactOptions = {}): Promise<void> {
  ...
  if (!existsSync(detailsPath)) {
    logger.error("details.json が見つかりません。先に `shirokuma-docs details` を実行してください。");
    return;
  }

  const detailsJson: DetailsJson = JSON.parse(readFileSync(detailsPath, "utf-8"));
  ...
  if (options.target) {
    ...
    // 全アイテムを分析
    results = Object.values(items).map(item =>
      analyzeImpact(item, dependencyGraph, maxDepth)
    );
  }
...
function buildDependencyGraph(items: Record<string, DetailItem>): DependencyGraph {
...
function findItem(
  ...
  // まずキーで検索
  if (items[target]) return items[target];
...
function analyzeImpact(
  ...
  const transitiveImpact: ImpactNode[] = [];
  ...
  // 間接的な依存（BFS）
  const queue: { key: string; depth: number }[] = Array.from(directDependents).map(k => ({
    key: k,
    depth: 1,
  }));
...
function findItemByKey(graph: DependencyGraph, key: string): boolean {
  return graph.has(key);
}

// ===== 出力生成 =====

function printImpactTable(results: ImpactResult[], target?: string): void {
  if (target) {
    // 単一アイテムの詳細表示
    ...
    // サマリー表示（影響が多いトップ20）
    console.log("\n📊 変更影響分析サマリー（影響が多い順・トップ20）\n");
    console.log("影響数 | 種類      | 名前");
    console.log("-------|-----------|------------------");
...
function generateImpactHtml(report: ImpactReport): string {
