 ...
 * test-cases コマンドハンドラ
 */
export async function testCasesCommand(options: TestCasesOptions): Promise<void> {
  ...
  const generatedPages = generateHierarchicalPages(
    allTestCases,
    summary,
    config.project.name,
    testCasesDir
 ...
 * Jest テストファイルを収集
 */
export async function collectJestFiles(
  projectPath: string,
  jestConfig?: { config?: string; testMatch?: string[] }
): Promise<string[]> {
 ...
 */
export async function collectPlaywrightFiles(
  projectPath: string,
  playwrightConfig?: { config?: string; testDir?: string }
): Promise<string[]> {
