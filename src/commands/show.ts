 ...
 */
export function detectItemType(input: DetectInput): DetectResult | null {
  ...
  if (hasPR) {
    const result: DetectResult = { type: "pr", data: pullRequest! };
    if (hasDiscussion) {
  ...
  // Issue
  if (hasIssue) {
    const result: DetectResult = { type: "issue", data: issue! };
    if (hasDiscussion) {
      result.ambiguous = { type: "discussion", data: discussion! };
    }
    return result;
...
export async function showCommand(
  ...
  const result = await runGraphQL<QueryResult>(GRAPHQL_QUERY_DETECT_TYPE, {
    owner,
    name: repo,
    number,
  }, { silent: true });
  ...
  if (options.verbose && detected.ambiguous) {
    logger.info(
      `Note: #${number} found as both ${detected.type} and ${detected.ambiguous.type}. Showing ${detected.type}.`
    );
  }
