 ...
 * パターンの最初の出現位置を取得
 */
function findFirstMatch(body: string, patterns: RegExp[]): number {
  let minIndex = -1;
 ...
 */
export const serverActionStructureRule: ServerActionStructureRule = {
  id: "server-action-structure",
  severity: "error",
  description:
    "Server Action must follow the order: auth -> CSRF (optional) -> Zod validation",
