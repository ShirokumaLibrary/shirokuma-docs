 ...
 */
export function findCodeStartIndex(content: string): number {
 ...
 * ファイルヘッダーからメタデータを抽出
 */
export function extractFileMetadata(content: string, codeStartIndex: number): FileMetadata {
  ...
  const toUndefinedIfEmpty = (arr: string[]): string[] | undefined => arr.length > 0 ? arr : undefined;
 ...
 */
export function parseFeatureMapTags(content: string, filePath: string): FeatureMapItem[] {
 ...
 */
export function parseFeatureMapTagsWithMetadata(content: string, filePath: string): ParseResult {
  ...
  if (headerJsdocMatch) {
    ...
    if (headerTags.screen || headerTags.component || headerTags.module || headerTags.serverAction !== undefined || headerTags.dbTable) {
      ...
      let headerItemName: string | undefined;
 ...
 * コメント後のコードからアイテム名を抽出
 */
export function extractItemName(code: string): string | undefined {
  // 関数/コンポーネント名を抽出
  const patterns = [
...
export function parseJSDocBlock(
  ...
  // アイテムタイプを判定
  let type: FeatureMapItem["type"] | undefined;
  let name: string | undefined = defaultName;
