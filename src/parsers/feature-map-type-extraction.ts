 ...
 * エクスポートされた型定義を抽出
 */
export function extractExportedTypes(content: string): TypeItem[] {
 ...
 */
export function extractPrecedingJSDoc(
  content: string,
  matchIndex: number,
  matchText: string
): { jsdoc: string | null; sourceCode: string } {
  // マッチ位置より前の内容を取得
 ...
 * インターフェースのフィールドを抽出
 */
export function extractInterfaceFields(body: string): { name: string; type: string; description?: string }[] {
 ...
 * enumの値を抽出
 */
export function extractEnumValues(body: string): string[] {
  const values: string[] = [];
  const lines = body.split(",");
 ...
 */
export function extractExportedUtilities(content: string): UtilityItem[] {
 ...
 * 関数の引数文字列をパースする
 */
export function parseParams(paramsStr: string): { name: string; type: string }[] {
