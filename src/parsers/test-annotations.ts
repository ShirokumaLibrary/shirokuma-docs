  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (itMatch) {
      ...
      if (!line.match(/test\.describe/)) {
        ...
        // describe ドキュメントを収集
        const describeDocs: DescribeDoc[] = describeStack
 ...
 */
export function extractTestDocComment(
  lines: string[],
  ...
  const result: TestDocComment = {};
  const bdd: BddAnnotation = {};
 ...
 * テストカテゴリをパース
 */
export function parseTestCategory(value: string): TestCategory {
 ...
 */
export function extractFileDocComment(content: string): FileDocComment | null {
  const lines = content.split("\n");
  ...
  if (commentStartIndex === -1 || commentEndIndex === -1) return null;

  // コメント内容を解析
  const commentLines = lines.slice(commentStartIndex, commentEndIndex + 1);
  const result: FileDocComment = {};
 ...
 */
export function extractDescribeDocComment(
  lines: string[],
  describeLineIndex: number
): { testdoc?: string; purpose?: string; priority?: "high" | "medium" | "low" } | null {
  // 直前の行からコメント終了を探す
  let commentEndIndex = -1;
  ...
  // コメント内容を解析
  const commentLines = lines.slice(commentStartIndex, commentEndIndex + 1);
  const result: { testdoc?: string; purpose?: string; priority?: "high" | "medium" | "low" } = {};
      ...
      switch (tag) {
        ...
        case "priority":
          if (["high", "medium", "low"].includes(value.toLowerCase())) {
            result.priority = value.toLowerCase() as "high" | "medium" | "low";
          }
          break;
      }
    }
