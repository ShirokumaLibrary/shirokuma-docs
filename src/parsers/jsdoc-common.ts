 ...
 * ```
 */
export function parseJsDoc(jsDocBlock: string): JsDocParsed {
 ...
 */
function normalizeJsDocBlock(jsDocBlock: string): string[] {
  // /** と */ を除去
...
function extractTags(
  lines: string[],
  startIndex: number
): Map<string, JsDocTagValue> {
  const tags = new Map<string, JsDocTagValue>();
  ...
  const processCurrentTag = () => {
    ...
    if (tags.has(tagName)) {
      ...
      if (Array.isArray(existing) && Array.isArray(value)) {
        tags.set(tagName, [...existing, ...value] as JsDocTagValue);
      }
      // 単行タグの重複は上書き
      else {
 ...
 * タグの内容を種類に応じてパース
 */
function parseTagContent(tagName: string, lines: string[]): JsDocTagValue {
  // 単行タグ
 ...
 * 複数行タグの内容をパース
 */
function parseMultiLineContent(lines: string[]): string {
  return lines
    .filter((line) => line !== "")
    .join("\n")
    .trim();
 ...
 */
function parseListContent(lines: string[]): JsDocListItem[] {
  const items: JsDocListItem[] = [];
 ...
 */
function parseErrorCodeContent(lines: string[]): JsDocListItem[] {
  const items: JsDocListItem[] = [];
 ...
 */
function parseParamContent(lines: string[]): JsDocParamItem[] {
 ...
 */
export function getTagLines(parsed: JsDocParsed, tagName: string): string[] {
  const value = parsed.tags.get(tagName);
  if (typeof value === "string") {
    return value.split("\n").filter((l) => l.trim());
  }
...
export function getTagParams(
  ...
  if (Array.isArray(value) && value.length > 0 && typeof value[0] === "object" && "name" in value[0]) {
    return value as JsDocParamItem[];
  }
  return [];
}
 ...
 * タグが存在するかチェック
 */
export function hasTag(parsed: JsDocParsed, tagName: string): boolean {
 ...
 * すべてのタグ名を取得
 */
export function getTagNames(parsed: JsDocParsed): string[] {
 ...
 */
export function extractFileHeaderJsDoc(sourceCode: string): string {
 ...
 */
function getScreenOrComponentName(jsDocBlock: string): string | undefined {
 ...
 * @returns JSDocブロック（見つからない場合は空文字列）
 */
export function extractJsDocBefore(
  sourceCode: string,
  targetName: string
): string {
  ...
  for (const pattern of patterns) {
    ...
    if (jsDocStart === -1 || jsDocStart > jsDocEnd) continue;
 ...
 */
export function extractAllJsDocs(
  sourceCode: string
): Array<{ name: string; jsDoc: string; parsed: JsDocParsed }> {
  const results: Array<{ name: string; jsDoc: string; parsed: JsDocParsed }> = [];
 ...
 */
export function extractDbSchemaJsDocs(sourceCode: string): DbSchemaJsDocInfo {
  const result: DbSchemaJsDocInfo = {
    tables: new Map(),
    columns: new Map(),
    indexes: new Map(),
    parsed: new Map(),
 ...
 */
function extractTableJsDocs(
  sourceCode: string,
  tableDefStart: number
): { columns: Map<string, string>; indexes: Map<string, string> } {
  ...
  if (openBraceIndex === -1) return { columns, indexes };

  // 対応する } を探す（文字列・コメント考慮）
  const closeBraceIndex = findMatchingBrace(sourceCode, openBraceIndex);
  if (closeBraceIndex === null) return { columns, indexes };
 ...
 */
export function extractExportedComponentName(content: string): string | undefined {
