 ...
 */
export function parseDrizzleSchemaDir(
  schemaDir: string,
  logger?: Logger
): DrizzleSchemaResult {
...
export function parseDrizzleSchema(
  ...
  const pgTableRegex =
    /(?:\/\*\*[\s\S]*?\*\/\s*)?export\s+const\s+(\w+)\s*=\s*pgTable\s*\(\s*["'](\w+)["']\s*,\s*\{/g;
...
function extractColumns(
  ...
  if (columnsBody.includes("...timestamps")) {
    columns.push(
      { name: "created_at", variableName: "createdAt", type: "timestamp", nullable: false },
      { name: "updated_at", variableName: "updatedAt", type: "timestamp", nullable: false }
    );
  }
...
function extractForeignKeys(
  ...
  const refRegex =
    /(\w+)\s*:\s*\w+\s*\(\s*["'](\w+)["'][^)]*\)(?:[^.]*)?\.references\s*\(\s*\(\s*\)\s*=>\s*(\w+)\.(\w+)/g;
 ...
 * インデックス定義を抽出
 */
function extractIndexes(indexesBody: string): DrizzleIndex[] {
  const indexes: DrizzleIndex[] = [];

  if (!indexesBody.trim()) return indexes;
 ...
 * 定義の直前にある JSDoc を抽出
 */
function extractJsDocBefore(sourceCode: string, defStart: number): string {
  ...
  if (jsDocStart === -1 || jsDocStart > jsDocEnd) return "";
 ...
 * JSDoc から説明文を抽出
 */
function extractDescriptionFromJsDoc(jsDoc: string): string | undefined {
 ...
 */
function mapDrizzleTypeToSql(drizzleType: string, restDef: string): string {
 ...
 * ファイル名からカテゴリを推論
 */
function inferCategoryFromFile(fileName: string): string {
 ...
 */
export function toPortalDbSchema(result: DrizzleSchemaResult): {
