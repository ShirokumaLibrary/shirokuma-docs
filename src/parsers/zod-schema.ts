 ...
 */
function parseObjectFields(objectBody: string): ZodParameter[] {
  ...
  const fieldStarts: Array<{ name: string; index: number }> = [];
 ...
 */
function parseField(name: string, definition: string): ZodParameter | null {
  ...
  const validationMatch = definition.match(
    /\.(uuid|email|url|min|max)\s*\(\s*(?:\d+\s*,\s*)?["']([^"']+)["']/
  );
  if (validationMatch) {
    param.validation = {
 ...
 */
function extractEnumValues(definition: string): string[] {
 ...
 * ```
 */
export function findSchemaInFiles(
