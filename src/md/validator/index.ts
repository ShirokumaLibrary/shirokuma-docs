...
import type { ValidationResult, Issue } from '../types/validation.js';
...
export class Validator {
  ...
  private async validateDocument(document: Document, projectRoot: string): Promise<Issue[]> {
    const issues: Issue[] = [];
  ...
  private detectInternalLinks(content: string, filePath: string): Issue[] {
    const issues: Issue[] = [];
  ...
  private getLineContext(line: string, matchIndex: number, linkText: string, linkUrl: string): string {
  ...
  private async validateTemplateCompliance(document: Document): Promise<Issue[]> {
    const issues: Issue[] = [];
    const templateName = document.frontmatter.template as string;
