...
export class Extractor {
  ...
  private applyMapping(
    ...
    result.warnings.push(
      `Unmapped value: ${fieldName} = "${value}" (not found in ${mappingName})`
    );
    return value;
  }
    ...
    if (this.config.extraction?.templates?.[documentType]) {
      ...
      for (const [key, value] of Object.entries(template)) {
        if (key === 'depends_on' && Array.isArray(value)) {
          ...
          frontmatter[key] = value.map((dep: string) =>
            this.substituteVariables(dep, extractedFields)
          );
        } else {
   ...
   */
  private substituteVariables(template: string, fields: Record<string, any>): string {
