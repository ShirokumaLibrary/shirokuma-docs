...
export class TokenOptimizer {
  ...
  private registerBuiltinRules() {
    ...
    this.rules.push({
      ...
      detect: (content: string, filePath: string) => {
        ...
        const phrases: Array<{ before: string; after: string; savings: number }> = [
  ...
  generateReport(issues: OptimizationIssue[]): OptimizationReport {
    const issuesBySeverity: Record<'error' | 'warning' | 'info', OptimizationIssue[]> = {
      error: [],
      warning: [],
      info: [],
    };

    const issuesByRule: Record<string, OptimizationIssue[]> = {};
   ...
   */
  private generateRecommendations(issues: OptimizationIssue[], totalSavings: number): string[] {
  ...
  formatReportMarkdown(report: OptimizationReport): string {
    ...
    // Group by file
    const fileGroups = new Map<string, OptimizationIssue[]>();
