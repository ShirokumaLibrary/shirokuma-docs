  ...
  try {
    ...
    if (options.suggestFixes) {
      ...
      if (issues.length === 0) {
        ...
        console.log(chalk.gray('─'.repeat(50)));
        console.log(
          `Total: ${chalk.red(errors.length + ' errors')}, ${chalk.yellow(warnings.length + ' warnings')}, ${chalk.blue(infos.length + ' info')}`
        );
...
function formatIssue(issue: any, verbose?: boolean): string {
...
async function runTokenOptimization(
  config: any,
  sourceDir: string,
  options: LintOptions,
  spinner: any
  ...
  const patterns = config.build.include.map((p: string) => path.join(sourceDir, p));
  const excludePatterns = config.build.exclude.map((p: string) => path.join(sourceDir, p));
