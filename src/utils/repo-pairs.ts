...
import { loadGhConfig, type RepoPairConfig, type GhConfig } from "./gh-config.js";
 ...
 */
export function getAllRepoPairs(config?: GhConfig): RepoPairConfig[] {
  const cfg = config ?? loadGhConfig();
  if (!cfg.repoPairs) return [];
 ...
 */
export function getRepoPair(alias: string, config?: GhConfig): RepoPairConfig | null {
 ...
 */
export function detectCurrentRepoPair(config?: GhConfig): RepoPairConfig | null {
  ...
  return pairs.find(
    p => p.private === currentFullName || p.public === currentFullName
  ) ?? null;
}
 ...
 */
export function resolveCrossRepo(alias: string, config?: GhConfig): { owner: string; name: string } | null {
  const cfg = config ?? loadGhConfig();
  if (!cfg.crossRepos) return null;
 ...
 */
export function getAllCrossRepos(config?: GhConfig): Array<{ alias: string; repo: string }> {
  const cfg = config ?? loadGhConfig();
  if (!cfg.crossRepos) return [];
 ...
 */
export function validateCrossRepoAlias(alias: string, config?: GhConfig): string | null {
  const cfg = config ?? loadGhConfig();
 ...
 */
export function isPrivateRepo(repoFullName: string, config?: GhConfig): boolean {
  const pairs = getAllRepoPairs(config);
 ...
 */
export function isPublicRepo(repoFullName: string, config?: GhConfig): boolean {
  const pairs = getAllRepoPairs(config);
 ...
 */
export function getMergedExcludePatterns(
  alias: string,
  projectPath: string,
  config?: GhConfig,
): string[] {
