 ...
 */
function resolveLocaleFromEnv(): Locale | undefined {
 ...
 */
export function setLocaleFromConfig(configLocale?: string): void {
 ...
 */
function loadDict(locale: Locale): MessageDict {
  ...
  try {
    ...
    const dict = JSON.parse(content) as MessageDict;
    cache.set(locale, dict);
    return dict;
  } catch {
    // Fallback to empty dict if file not found
    const empty: MessageDict = {};
 ...
 */
function getNestedValue(dict: MessageDict, key: string): string | undefined {
  const parts = key.split(".");
  let current: string | MessageDict = dict;
