...
// Protocol-specific badge labels
...
function CategoryCard({
  ...
  const categoryIndexLink = `/apps/${appId}/tools/${category}/`;
        ...
        <div className="space-y-2">
          {tools.slice(0, 5).map((tool) => {
            ...
            return (
              <Link
                key={tool.name}
                href={`/apps/${appId}/tools/${tool.name}`}
...
export function ApiToolsClient({ data, appId, protocol }: ApiToolsClientProps) {
  const [activeTab, setActiveTab] = useState<CategoryType | null>(null);
  const [mounted, setMounted] = useState(false);

  const badgeLabel = protocolBadge[protocol] || protocolBadge.default;
  ...
  return (
    <div className="space-y-8">
      ...
      {activeTab === null ? (
        // Show all categories
        <CategoryContent toolsByCategory={toolsByCategory} appId={appId} badgeLabel={badgeLabel} />
      ) : (
